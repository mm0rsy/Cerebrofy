...
# Local fixture strings representing v1 (warn-only) and v2 (hard-block) hook content.
_HOOK_SCRIPT_V1: str = (
...
def test_replace_hook_block_replaces_v1_with_v2() -> None:
    ...
    content = f"#!/bin/sh\n{_HOOK_SCRIPT_V1}"
    new_content = _replace_hook_block(content, _HOOK_SCRIPT_V2)
...
def test_replace_hook_block_appends_when_no_block() -> None:
    ...
    original = "#!/bin/sh\necho 'existing hook'\n"
    new_content = _replace_hook_block(original, _HOOK_SCRIPT_V2)
